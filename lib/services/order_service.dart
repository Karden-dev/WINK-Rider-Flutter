...
class OrderService {
  ...
  static const String _apiBaseUrl = "http://localhost:3000/api/rider"; // Base API Livreur
  ...
  Future<void> updateOrderStatus({
    ...
    try {
      ...
      await _dio.put('/api/orders/$orderId/status', data: payload); // Note: utilise l'endpoint /api/orders
  ...
  Future<void> declareReturn(int orderId) async {
    try {
      // L'endpoint est POST /api/orders/:id/declare-return
      await _dio.post('/api/orders/$orderId/declare-return', data: {'comment': 'Déclaré depuis app Flutter'});
    } on DioException catch (e) {
