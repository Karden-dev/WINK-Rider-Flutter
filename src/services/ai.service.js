...
// Utilisation de l'original GoogleGenAI (correct pour la v1.x)
...
const processRequest = async (userInfo, userMessage) => {
    ...
    // NOMS DE MODÈLES CORRIGÉS
    let modelToUse = 'gemini-1.5-flash-latest'; // Par défaut, modèle rapide et économique
    ...
    if (userInfo.role === 'admin' || 
        ...
        modelToUse = 'gemini-1.5-pro-latest'; // Modèle le plus intelligent
    }
    ...
    try {
        ...
        // Utilisation de 'ai.getGenerativeModel' (correct pour la v1.x)
        ...
        // Logguer l'erreur dans notre fichier de débogage
        try {
            const fs = require('fs');
            const path = require('path');
            const logFilePath = path.join(__dirname, '..', 'whatsapp_debug.log');
            // Log de l'erreur Gemini
            fs.appendFileSync(logFilePath, `ERREUR API GEMINI (${modelToUse}): ${error.message}\n`, 'utf8');
        } catch (e) {}
 ...
 */
const generateText = async (prompt, model = 'gemini-1.5-pro-latest') => {
