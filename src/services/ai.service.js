...
// Ces modèles permettront à l'IA d'accéder à l'intégralité de winkdb
...
const processRequest = async (userInfo, userMessage) => {
    ...
    // NOTE: Les modèles (UserModel, OrderModel) sont maintenant utilisables 
    // car ils ont été initialisés par app.js avant cet appel.
    
    // Exemple de récupération de contexte pour un livreur
    if (userInfo.role === 'livreur' && userInfo.id) {
        // const riderStats = await RiderModel.findRiderStats(userInfo.id); 
        // businessContext.personal_data = riderStats;
    }
    // ... (Ajouter d'autres logiques de récupération de contexte pour les 'shops' ou 'admins') ...
